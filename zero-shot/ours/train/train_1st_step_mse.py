...
from utils.model_utils import MaskedMSELoss, ICTransformer
from utils.plot_utils import plot_loss, plot_lr, plot_series
from utils.generate_utils import TimeSeriesDataset
...
def train_one_epoch(model, loss_fn, optimizer, scheduler, train_loader, device):
    ...
    for i, data in enumerate(train_loader):
        ...
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.) # comment for the first version without gradient clipping
        optimizer.step()
        scheduler.step()
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if epoch % 2 == 0 or epoch == epochs - 1:
            with torch.no_grad():
                ...
                axs[0].set_title(f'Train loss = {np.round(np.array(train_loss[-1]), 5)}. Val loss = {np.round(np.array(val_loss[-1]), 5)}.')
