...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, path, model_params):
        score = -val_loss
        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(val_loss, model, path, model_params)
    ...
    def save_checkpoint(self, val_loss, model, path, model_params):
        if self.verbose:
            print(f'Validation loss decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...')
        torch.save({'model_params': model_params, 'model_state_dict': model.state_dict()}, path + '/' + 'checkpoint.pth')
...
def vali(model, vali_data, vali_loader, criterion, args, device, ii, training=False):
    total_loss = []
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(vali_loader)):
            ...
            if training: 
                ...
                assert args.pred_len >= args.target_pred_len
                if args.pred_len == args.target_pred_len:
                    outputs = outputs[:, -args.pred_len:, :]
                    batch_y = batch_y[:, -args.pred_len:, :].to(device)
                elif args.pred_len > args.target_pred_len:
                    outputs = outputs[:, -args.pred_len:-args.pred_len+args.target_pred_len, :]
                    batch_y = batch_y[:, -args.pred_len:-args.pred_len+args.target_pred_len, :].to(device)
...
def test(model, test_data, test_loader, args, device, ii):
    ...
    preds = test_loader.dataset.inverse_source_transform(preds)
