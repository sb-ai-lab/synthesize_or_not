...
class Dataset_ETT_hour(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 ...
                 percent=100, max_len=-1, train_all=False, source_scaler=False):
        ...
        self.dataset_scaler = deepcopy(source_scaler)
    ...
    def __read_data__(self):
        ...
        if self.source_scaler:
            self.dataset_scaler.fit(data.reshape(-1, 1))
            data_default_scale = self.dataset_scaler.transform(data.reshape(-1, 1))
            data_source_scale = self.source_scaler.inverse_transform(data_default_scale).reshape(data.shape)
        else:
            data_source_scale = data
        ...
        self.data_x = data_source_scale[border1:border2]
    ...
    def inverse_source_transform(self, data):
        if self.source_scaler:
            default_scale = self.source_scaler.transform(data.reshape(-1, 1))
            initial_scale = self.dataset_scaler.inverse_transform(default_scale).reshape(data.shape)
...
class Dataset_Custom(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=0, freq='h',
                 percent=10, max_len=-1, train_all=False, source_scaler=False):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4
...
class Dataset_TSF(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 ...
                 percent=10, max_len=-1, train_all=False, source_scaler=None):
    ...
    def __read_data__(self):
        ...
        for i in range(len(timeseries)):
            ...
            curr_len = border2s[self.set_type] - max(border1s[self.set_type], 0) - self.seq_len + 1
            #curr_len = 1
    ...
    def __getitem__(self, index):
        if self.set_type == 2:
            ...
            s_begin = train_len - self.seq_len
            s_end = s_begin + self.seq_len
            r_begin = s_end
